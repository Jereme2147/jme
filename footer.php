</main>
 <footer>
                <div class="footer-phone">
                    <div class="phone">
                        <h2>-828-355-9024-</h2>
                    </div>
                    <div class="footer-service">
                        <a href="<?php echo get_home_url()?>/contact">Click here to schedule service</a>
                    </div>
                </div>
                <div class="footer-links-social-container">
                    <div class="footer-links">
                        <ul>
                            <a href="<?php echo get_home_url();?>/about"><li>About</li></a>
                            <a href="<?php echo get_home_url();?>/services"><li>Services</li></a>
                            <a href="<?php echo get_home_url();?>/our-vendors"><li>Vendors</li></a>
                            <a href="<?php echo get_home_url();?>/contact"><li>Contact</li></a>
                        </ul>
                    </div>
                    <div class="footer-social">
                        <a href="#"><img src="<?php echo get_template_directory_uri(); ?>/images/social/facebook.svg" alt=""> </a>
                        <a href="#"><img src="<?php echo get_template_directory_uri(); ?>/images/social/instagram.svg" alt=""></a>
                        <a href="#"><img src="<?php echo get_template_directory_uri(); ?>/images/social/linkedin.svg" alt=""></a>
                        <a href="#"><img src="<?php echo get_template_directory_uri(); ?>/images/social/mail.svg" alt=""></a>
                        <a href="#"><img src="<?php echo get_template_directory_uri(); ?>/images/social/phone.svg" alt=""></a>
                    </div>
                </div>
                <div class="footer-developer">
                    <a href="www.jeremedaniels.com">Web Design by Jereme Daniels</a>
                </div>

            </footer>
    </div>
    <?php wp_footer(); ?>
</body>
</html>